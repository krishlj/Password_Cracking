...
Hashcat uses the following basic syntax: hashcat -m <hash\_type> -a <attack\_mode> hashfile wordlist
...
**-m** <hash\_type> specifies the hash-type in numeric format. For example, -m 1000 is for NTLM.



**-a** <attack\_mode> specifies the attack-mode. For example, -a 0 is for straight, i.e., trying one password from the wordlist after the other.
...
unshadow local\_passwd local\_shadow > youname.txt
...
zip2john zipfile.zip > yourname\_hash.txt



Cracking

john --wordlist=/usr/share/wordlists/rockyou.txt yourname\_hash.txt
...
/usr/sbin/rar2john rarfile.rar > yourname\_hash.txt
...
usr/share/john/ssh2john.py id\_rsa > yourname\_rsa\_hash.txt
...
john --wordlist=/usr/share/wordlists/rockyou.txt yourname\_rsa\_hash.txt
